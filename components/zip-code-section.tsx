...
const ZipCodeSection: React.FC<ZipCodeSectionProps> = ({ onZipResult }) => {
  ...
  const { xfinityData, frontierFiberData, frontierCopperData, optimumData, metronetData, kineticData } = useZipData()
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    providersData.forEach((providerData) => {
      const { name, city: providerCity, state: providerState, service, zipCodes } = providerData
      const zipMatch = zipCodes.includes(zipCode)
      debugInfo[name] = zipMatch ? 1 : 0

      if (zipMatch) {
        availableProviders.push(name + (service ? ` (${service})` : ""))
        if (!city && providerCity) city = providerCity
        if (!state && providerState) state = providerState
      }
    })

    if (!availableProviders.includes("EarthLink")) {
      availableProviders.push("EarthLink (Nationwide)")
      debugInfo["EarthLink"] = 1
    }
    if (!availableProviders.includes("DirecTV")) {
